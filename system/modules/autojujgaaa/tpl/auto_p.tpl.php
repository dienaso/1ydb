...
<div id="zhuangtai" style="width:100%; height:30px; border:1px solid #CCCCCC; line-height:30px; text-align:center; font-size:18px;<?php if($xml->oo == "on"){?> color:#009933;<?php }else{?> color:red;<?php }?>">
	<?php if($xml->oo == "on"){?> 正在运行<?php }else{?> 停止运行<?php }?>
</div>
	<div class="div" >
	指定间隔时间段(秒)：
	...
	<div >
		<span style="color:#FF0000;">*</span> <span style="color:#CC6633;">指定购买小时段：</span><span style="display:none;"><?php echo $timeperiod; ?></span>
		<table id="table">
		<?php for($i=0;$i<24;$i++){ ?>
				<?php if($i%6 ==0){ ?>	
						<tr class="tr">
	...
	<div class="div" style="height:auto;">指定购买商品：<label id="shopid"><?php echo $shopid; ?></label></div>
			...
			if(shopid == '' ||  shopid ==null ){
				...
				$(".cb_value").each(function(){
					if(shopidstr == null){
						...
						shopidstr = shopidstr+"-"+$(this).val();
					}
				});
				$("#shopid").html(shopidstr);
				$(".cb").attr("checked","checked");
				
			}else{
				
				$(".cb_value").each(function(){
					if(shopid.indexOf($(this).val())  < 0  ){
			...
			$(".cb_value").each(function(){
				...
				for(var i = 0; i<shopidarray.length;i++ ){
					if(id == shopidarray[i]){
						...
						tmpshopid.push(shopidarray[i]);
					}
				}
				shopidarray = tmpshopid;
			});
			$("#shopid").html(shopidarray.join("-"));
