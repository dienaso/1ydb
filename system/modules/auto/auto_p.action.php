...
<timeperiod>'.$xml['timeperiod'].'</timeperiod>
	...
	public function buyshop($xml,$shopinfo,$member){
		//file_put_contents("buy.txt",date("Y-m-d H:i:s",time()) . "\r\n" , FILE_APPEND);
		//购买商品数量
		$shopnum = rand(-100,10);
		if($shopnum <= -40){
			$shopnum = 1;
		...
		if($shopinfo['zongrenshu'] != $shopinfo['canyurenshu']){
			...
			$randip = $_SERVER['HTTP_CLIENT_IP'];
			$user_ip = $member['user_ip'];
			if(!$user_ip){
				$this->db->Query(" UPDATE `@#_member` SET  `user_ip` = '$randip' WHERE `uid` = '$user_id' ");
			}
			//调用购买商品接口
	...
	public function xcaction(){
		...
		if($tp_tmp == 1){
			...
			$min_uid = intval($useridarray[0]);
			$max_uid = intval($useridarray[1]);
			//$user_id = $this->getuser();
			#查询用户是否是批量注册的用户
			$member = $this->db->GetOne("SELECT * FROM `@#_member` WHERE `uid` >= '$min_uid' and `uid` <= '$max_uid' and  `auto_user` = '1' order by rand()");
	...
	*/	
	...
	public function xhaction(){
		...
		while(true){
			...
			unset($xml);
			#休眠时间
			//$ss = sleep(rand($times,$endtimes));
			usleep(rand($times,$endtimes) * 1000000);
			//file_put_contents("aaa.txt",rand($times,$endtimes) . "秒" .date("Y-m-d H:i:s",time()) . "\r\n",FILE_APPEND);
