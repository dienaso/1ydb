...
class finduser extends SystemAction {
	...
	public function findpassword(){
		if(isset($_POST['submit'])){			
			...
			header("location:".WEB_PATH."/mobile/finduser/find".$regtype."check"."/"._encrypt($name));
		}
		$title="找回密码";
	...
	public function findsendmobile(){
		...
		if($times > 120){
			...
			_message("正在重新发送...",WEB_PATH."/mobile/finduser/findmobilecheck/"._encrypt($member['mobile']),2);				
		}else{
			_message("重发时间间隔不能小于2分钟!",WEB_PATH."/mobile/finduser/findmobilecheck/"._encrypt($member['mobile']));
		}		
	}
	public function findmobilecheck(){
		...
		if($member['passcode']==-1){
			...
			header("location:".WEB_PATH."/mobile/finduser/findmobilecheck/"._encrypt($member['mobile']));
			exit;
		}		
		if(isset($_POST['submit'])){
			...
			header("location:".WEB_PATH."/mobile/finduser/findok/".$urlcheckcode,2);
	...
	public function findsendemail(){
		...
		_message("正在重新发送...",WEB_PATH."/mobile/finduser/findemailcheck/".$this->segment(4),2);				
		exit;	
	}	
