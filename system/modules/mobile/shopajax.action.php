...
/* 		if(ROUTE_A!='userphotoup' and ROUTE_A!='singphotoup'){
	...
	public function getUserBuyList(){
	   ...
	   if($state==-1){
	     //参与购买的商品 全部...
		  $shoplist=$this->db->GetList("select *,sum(gonumber) as gonumber from `@#_member_go_record` a left join `@#_shoplist` b on a.shopid=b.id where a.uid='$member[uid]' GROUP BY b.id ");

		  $shoplistall['listItems']=$this->db->GetList("select *,sum(gonumber) as gonumber from `@#_member_go_record` a left join `@#_shoplist` b on a.shopid=b.id where a.uid='$member[uid]' GROUP BY a.id order by a.time desc limit $FIdx,$EIdx " );
		}elseif($state==1){
		  //参与购买的商品 进行中...
		  $shoplist=$this->db->GetList("select *,sum(gonumber) as gonumber from `@#_member_go_record` a left join `@#_shoplist` b on a.shopid=b.id where a.uid='$member[uid]' and b.q_end_time is null GROUP BY a.id  " );


		  $shoplistall['listItems']=$this->db->GetList("select *,sum(gonumber) as gonumber from `@#_member_go_record` a left join `@#_shoplist` b on a.shopid=b.id where a.uid='$member[uid]' and b.q_end_time is null GROUP BY a.id order by a.time desc limit $FIdx,$EIdx " );
		}else{
		  //参与购买的商品 已揭晓...
		  $shoplist=$this->db->GetList("select *,sum(gonumber) as gonumber from `@#_member_go_record` a left join `@#_shoplist` b on a.shopid=b.id where a.uid='$member[uid]' and b.q_end_time is not null GROUP BY a.id " );


		  $shoplistall['listItems']=$this->db->GetList("select *,sum(gonumber) as gonumber from `@#_member_go_record` a left join `@#_shoplist` b on a.shopid=b.id where a.uid='$member[uid]' and b.q_end_time is not null GROUP BY a.id order by a.time desc limit $FIdx,$EIdx " );
		}
		...
		if(!empty($shoplistall['listItems'])){
		   ...
		   foreach($shoplistall['listItems'] as $key=>$val){
			  ...
			  $shoplistall['listItems'][$key]['q_end_time']=microt($val['q_end_time']);
			    ...
			    $shoplistall['listItems'][$key]['id']=$juid['id'];
