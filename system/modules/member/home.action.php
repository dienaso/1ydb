...
class home extends base {
	...
	public function modify(){
		...
		$member_weixin=$this->db->GetOne("select * from `@#_member_wxband` where `b_uid`=$member[uid]");
...
//夺宝币、经验添加			
	...
	public function usermodify(){
		...
		if(isset($_POST['submit'])){			
			...
			//夺宝币、经验添加
			$isset_user=$this->db->GetOne("select `uid` from `@#_member_account` where (`content`='手机认证完善奖励' or `content`='完善昵称奖励') and `type`='1' and `uid`='$member[uid]' and (`pay`='经验' or `pay`='夺宝币')");	
			if(!$isset_user){			
				$config = System::load_app_config("user_fufen");//夺宝币/经验
				$time=time();

				$this->db->Query("insert into `@#_member_account` (`uid`,`type`,`pay`,`content`,`money`,`time`) values ('$member[uid]','1','夺宝币','完善昵称奖励','$config[f_overziliao]','$time')");
	...
	public function userfufen(){
		$member=$this->userinfo;	
		$uid = $member['uid'];
		$title="账户夺宝币 - "._cfg("web_name");
	
		$total=$this->db->GetCount("select * from `@#_member_account` where `uid`='$uid' and `pay` = '夺宝币'");
		$page=System::load_sys_class('page');
		if(isset($_GET['p'])){$pagenum=$_GET['p'];}else{$pagenum=1;}	
		$page->config($total,20,$pagenum,"0");		
		$account = $this->db->GetPage("select * from `@#_member_account` where `uid`='$uid' and `pay` = '夺宝币' ORDER BY time DESC",array("num"=>20,"page"=>$pagenum,"type"=>1,"cache"=>0));
			...
			if ( $member['sign_in_date'] == date('Y-m-d') ) {
				...
				if ( $member['sign_in_date'] == date('Y-m-d',strtotime('-1 day')) ){
					...
					if ( $big ) {
						...
						_message("签到成功，成功领取{$time_score}夺宝币。<br />恭喜您获得{$time_day_score}夺宝币的大礼包。<br />您的当前夺宝币为{$score}",WEB_PATH."/member/home/qiandao",10);
					} else {
						_message("签到成功，成功领取{$time_score}夺宝币。<br />您的当前夺宝币为{$score}。<br />再连续签到".($time_day-$sign_in_time)."天就能领取大礼包啦，加油！！！",WEB_PATH."/member/home/qiandao");
					}
					...
					$mysql_model->Query("INSERT INTO `@#_member_account` (`uid`, `type`, `pay`, `content`, `money`, `time`) VALUES ('".$member['uid']."', '1', '夺宝币', '每日签到', '$time_score', '".time()."')");
					$mysql_model->Query("UPDATE `@#_member` SET score='".$score."',sign_in_time='".$sign_in_time."', sign_in_time_all='".$sign_in_time_all."', sign_in_date='".$sign_in_date."' where uid='".$member['uid']."'");
					_message("签到成功，成功领取{$time_score}夺宝币。<br />您的当前夺宝币为{$score}",WEB_PATH."/member/home/qiandao");
				}
			}
			die;
		}
