...
class user extends base {
	...
	function wxlogin(){
		...
		$redirect_uri = urlencode("".$wx_set['back']."/?member/user/wx_callback/".$code."/");
		$wxurl = "https://open.weixin.qq.com/connect/qrconnect?appid=".$wx_set['appid']."&redirect_uri=$redirect_uri&response_type=code&scope=snsapi_login&state=$state#wechat_redirect";
    ...
    function wx_callback(){
         session_start();
		 if($_GET["state"] != $_SESSION["wxState"]){ _message("登录验证失败!","".$wx_set['back']."/?/member/user/login");  }
		...
		$unionid = $jsondecode["unionid"];
         if(empty($unionid)){
         	_message("绑定出错，请联系管理员。");die;
         }
		 $access_token =$jsondecode["access_token"];
		 $response= file_get_contents("https://api.weixin.qq.com/sns/userinfo?access_token=$access_token&openid=$unionid");
		 $jsondecode = json_decode($response,true);
		 $nickname = $jsondecode["nickname"];
		 $tximg = $jsondecode["headimgurl"];
		 $go_user_info = $this->db->GetOne("select * from `@#_member_band` where `b_code` = '$unionid' and `b_type` = 'weixin' LIMIT 1");
		 ...
		 if($bb_code!=$unionid){
		  ...
		  $IMG = file_get_contents($tximg); 
			...
			$this->db->Query("INSERT INTO `@#_member_band` (`b_uid`, `b_type`, `b_code`, `b_time`) VALUES ('$uid', 'weixin', '$unionid', '$go_user_time')");
			...
			$callback_url =  WEB_PATH."/member/home/mobilechecking";
	...
	function wxband(){
		...
		$state  = md5(uniqid(rand(), TRUE));
		$_SESSION["wxState"]	=   $state;
		$redirect_uri = urlencode("http://v6.yungoutest.com/?member/user/wxband_callback/".$code."/");
		$wxurl = "https://open.weixin.qq.com/connect/qrconnect?appid=wx1a6a6271064810cb&redirect_uri=$redirect_uri&response_type=code&scope=snsapi_login&state=$state#wechat_redirect";
		header("Location: $wxurl");
	}



    function wxband_callback(){
         session_start();
		 if($_GET["state"] != $_SESSION["wxState"]){ _message("登录验证失败!","http://v6.yungoutest.com/?/member/user/login");  }

		 $code = $_GET["code"];
	     $procode = $this->segment(4);
	     file_put_contents('t.txt', "\n\r\r\n-----procode:".$procode,FILE_APPEND);
	     $response = file_get_contents("https://api.weixin.qq.com/sns/oauth2/access_token?appid=wx1a6a6271064810cb&secret=cce48f281a0f8efce3c69f0d0c434c7d&code=$code&grant_type=authorization_code");
	     $jsondecode = json_decode($response,true);
		...
		$uid=intval(_encrypt(_getcookie("uid"),'DECODE'));
         ...
         $wxband = $member['wxband'];
		if(!empty($wxband)){
         	_message("您已经绑定啦！",WEB_PATH."/member/home");
			
         }else{
		if(empty($uid)){
			_message("未登录，无权绑定！",WEB_PATH."/member/home");
         	
			
         }else{
			$this->db->Query("INSERT INTO `@#_member_band` (`b_uid`, `b_type`, `b_code`) VALUES ('$uid', 'weixin', '$unionid')");
			$bands = trim($memberones['wxband'],",");			
			$bands = $bands."weixin";
			$this->db->Query("UPDATE `@#_member` SET `wxband` = '$bands' where `uid` = '$uid' limit 1");
		...
		if($timec < (3600*24)){	
				...
				if($member['yaoqing']){
							...
							if($fili_cfg['f_visituser']){							
								$this->db->Query("insert into `@#_member_account` (`uid`,`type`,`pay`,`content`,`money`,`time`) values ('$yaoqinguid','1','夺宝币','邀请好友奖励','$fili_cfg[f_visituser]','$time')");
							}						
